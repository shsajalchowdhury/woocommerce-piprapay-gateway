...
function piprapay_init_gateway_class()
{
    ...
    class WC_PIPRAPAY_Gateway extends WC_Payment_Gateway
    {
        ...
        /**
         * Verify API key from webhook request headers.
         * Checks both old (mh-piprapay-api-key) and new (MHS-PIPRAPAY-API-KEY) formats.
         *
         * @param array $headers
         * @return bool
         */
        private function verify_webhook_auth($headers) {
            ...
            $header_keys = ['mh-piprapay-api-key', 'Mh-Piprapay-Api-Key', 'mhs-piprapay-api-key', 'MHS-PIPRAPAY-API-KEY'];
            ...
            if (empty($received_api_key)) {
                $server_keys = ['HTTP_MH_PIPRAPAY_API_KEY', 'HTTP_MHS_PIPRAPAY_API_KEY'];
            ...
            return hash_equals($this->apikey, (string) $received_api_key);
        }

        /**
         * Check if an order has already been paid (idempotency guard).
         *
        ...
        private function verify_payment_amount($order, $verification) {
            ...
            return abs($order_total - $verified_total) <= 0.01;
        }
        ...
        public function handle_webhook()
        {
            ...
            // Security: enforce API key auth for ALL webhook requests.
            ...
            if($this->piprapay_version == 'old'){
                ...
                // Security: idempotency check to prevent duplicate/replayed payments.
                ...
                if ($verification['status'] === 'completed') {
                    ...
                    if (!$this->verify_payment_amount($order, $verification)) {
                        $order->update_status('failed', __('Payment amount verification failed.', 'piprapay-gateway'));
                        status_header(400);
                        wp_send_json_error(['message' => 'Amount mismatch.']);
                        exit;
                    }

                    // Security: verify currency matches order currency.
                    if (!$this->verify_payment_currency($order, $verification)) {
                        $order->update_status('failed', __('Payment currency verification failed.', 'piprapay-gateway'));
                        status_header(400);
                ...
                // Security: removed unauthenticated $_GET['transaction_ref'] fallback.
